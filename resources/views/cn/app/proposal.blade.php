@extends('layouts.appcn')
@section('title', 'Globinc - Passive income on the crypto exchange.')
@section('meta', 'Passive income✔ making money on the Internet ✔ buy bitcoin ✔ everything about the crypto exchange - Globinc, your way to success.')
@section('keywords', 'Passive income | Globinc')
@section('breadcrumbs')
@endsection
@section('content')
    <div class="header-banner bg-theme-grad-s2">
        <div class="nk-banner">
            <div class="banner ">
                <div class="banner-wrap">
                    <div class="container">
                        <div class="row justify-content-center align-items-center">
                            <div class="col-xl-6 col-lg-9">
                                <div class="banner-caption cpn tc-light text-center">
                                    <div class="section-head section-head-s2">
                                        <h2 class="title title-xl" title="PARTNERSHIP" style="color: #fff;">合作夥伴計劃</h2>
                                        <p style="font-size: 1.4rem; color: #dabd75;"><span style="color: white; font-weight: 900;">Globinc</span><span  style="color: white; "></span> 這是您獲得成功和財務自由的機會。</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div><!-- .nk-banner -->
        <div class="nk-ovm nk-ovm-repeat nk-ovm-fixed shape-i">
            <div class="ovm-line"></div>
            <div class="nk-ovm shape-b"></div>
        </div>
    </div>


    </header>

    <main class="nk-pages">
        <section class="section bg-white" id="mvp">

            <div class="container">
                <div class="row justify-content-center text-center">
                    <div class="col-lg-6">
                        <div class="section-head section-head-s2">
                            <h2 class="title title-xl" title="Globinc">合作计划条款</h2>
                            <p>我们提供有效的工作工具，以及丰厚的奖励和职业机会奖金。</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="container container-xxl">
                <div class="nk-block">
                    <div class="row align-items-center justify-content-center justify-content-xl-between gutter-vr-30px">
                        <div class="col-xxl-6 col-xl-6 col-lg-8">
                            <div class="nk-block-img nk-block-plx">
                                <img src="{{asset('../images/part.png')}}">
                            </div>
                        </div>
                        <div class="col-xxl-5 col-xl-6 col-lg-8">
                            <div class="nk-block-text">
                                <div class="feature feature-inline">
                                    <div class="feature-icon feature-icon-md">
                                        <em class="icon icon-xs icon-circle fas fa-check"></em>
                                    </div>
                                    <div class="feature-text">
                                        <p>
                                            购买 GLOBAL 关税套餐时，以美元当前汇率固定资金，并以 USDT 付款。</p>
                                    </div>
                                </div>
                                <div class="feature feature-inline">
                                    <div class="feature-icon feature-icon-md">
                                        <em class="icon icon-xs icon-circle fas fa-check"></em>
                                    </div>
                                    <div class="feature-text">
                                        <p>套餐每周固定一天入账一次，提现手续费为2.5%。</p>
                                    </div>
                                </div>
                                <div class="feature feature-inline">
                                    <div class="feature-icon feature-icon-md">
                                        <em class="icon icon-xs icon-circle fas fa-check"></em>
                                    </div>
                                    <div class="feature-text">
                                        <p>处理申请的时限为72小时，如需提交新的提款申请，必须处理之前的提款申请。</p>
                                    </div>
                                </div>
                                <div class="text-center text-sm-left">
                                    <a href="https://my.globinc.com/order/main" class="btn btn-round btn-primary">投資</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </section>

        <div class="nk-preview" style="padding-top: 50px;background-color: #fff;">

            <div class="tab-content nk-preview-content">

                <!-- Copy from here -->
                <section class="section"  style="
  border-radius: 40px; /* The higher the pixel, the greater the curve */
  box-shadow: 13px 13px 20px #cbced1,
             -13px -13px 20px #ffffff;">

                    <div class="container con">
                        <!-- Section Head @s -->
                        <div class="section-head text-center wide-auto">
                            <h2 class="title title-xl">推荐奖金表
                                <p class="lead" style="font-size: 1.4rem; color: #dabd75;">金钱应该工作和利润。</p>
                            </h2>
                        </div><!-- .section-head @e -->
                        <!-- Block @s -->
                        <div class="nk-block" style="">
                            <div class="tbl-header">
                                <table cellpadding="0" cellspacing="0" border="0">
                                    <thead>
                                    <tr>
                                        <th style="width:50%; color:#fff;">生意伙伴</th>
                                        <th style="width:25%; color:#fff;">附加金额，$</th>
                                        <th style="width:10%; color:#fff;">%</th>
                                        <th style="width:15%; color:#fff;">奖,$</th>
                                    </tr>
                                    </thead>
                                </table>
                            </div>
                            <div class="tbl-content">
                                <table cellpadding="0" cellspacing="0" border="0" style="color:#fff;">
                                    <tbody>
                                    <tr>
                                        <td  style="width:50%">夥伴</td>
                                        <td  style="width:25%">151</td>
                                        <td  style="width:10%;">5</td>
                                        <td  style="width:15%"></td>
                                    </tr>
                                    <tr>
                                        <td>領先的合作夥伴</td>
                                        <td>200</td>
                                        <td>5,5</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        <td>生意夥伴</td>
                                        <td>500</td>
                                        <td>6</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        <td>經理</td>
                                        <td>1 000</td>
                                        <td>6,5</td>
                                        <td>100</td>
                                    </tr>
                                    <tr>
                                        <td>高級經理</td>
                                        <td>2 000</td>
                                        <td>7</td>
                                        <td>-</td>
                                    </tr>
                                    <tr>
                                        <td>地區經理</td>
                                        <td>5 000</td>
                                        <td>7,5</td>
                                        <td>200</td>
                                    </tr>
                                    <tr>
                                        <td>執行經理</td>
                                        <td>10 000</td>
                                        <td>8</td>
                                        <td>200</td>
                                    </tr>
                                    <tr>
                                        <td>館長</td>
                                        <td>20 000</td>
                                        <td>8,5</td>
                                        <td>300</td>
                                    </tr>
                                    <tr>
                                        <td>高級策展人</td>
                                        <td>30 000</td>
                                        <td>9</td>
                                        <td>400</td>
                                    </tr>
                                    <tr>
                                        <td>區域策展人</td>
                                        <td>40 000</td>
                                        <td>9,5</td>
                                        <td>500</td>
                                    </tr>
                                    <tr>
                                        <td>執行策展人</td>
                                        <td>50 000</td>
                                        <td>10</td>
                                        <td>500</td>
                                    </tr>
                                    <tr>
                                        <td>總監助理</td>
                                        <td>100 000</td>
                                        <td>10,5</td>
                                        <td>750</td>
                                    </tr>
                                    <tr>
                                        <td>董事高級助理</td>
                                        <td>150 000</td>
                                        <td>11</td>
                                        <td>1 000</td>
                                    </tr>
                                    <tr>
                                        <td>區域總監助理</td>
                                        <td>200 000</td>
                                        <td>11,5</td>
                                        <td>2 000</td>
                                    </tr>
                                    <tr>
                                        <td>導向器</td>
                                        <td>300 000</td>
                                        <td>12</td>
                                        <td>3 000</td>
                                    </tr>
                                    <tr>
                                        <td>高級總監</td>
                                        <td>400 000</td>
                                        <td>12,5</td>
                                        <td>4 000</td>
                                    </tr>
                                    <tr>
                                        <td>地區經理</td>
                                        <td>500 000</td>
                                        <td>13</td>
                                        <td>5 000</td>
                                    </tr>
                                    <tr>
                                        <td>執行董事</td>
                                        <td>1 000 000</td>
                                        <td>13,5</td>
                                        <td>10 000</td>
                                    </tr>
                                    <tr>
                                        <td>國際總監</td>
                                        <td>5 000 000</td>
                                        <td>14</td>
                                        <td>20 000</td>
                                    </tr>
                                    <tr>
                                        <td>股東</td>
                                        <td>10 000 000</td>
                                        <td>14,5</td>
                                        <td>50 000</td>
                                    </tr>
                                    <tr>
                                        <td>共有人</td>
                                        <td>100 000 000</td>
                                        <td>15</td>
                                        <td>100 000</td>
                                    </tr>
                                    </tbody>
                                </table></div>
                            <br>
                            <style>
                                @media (max-width: 950px) {
                                    .con {
                                        max-width: 100%;
                                        font-size: 9px;
                                    }
                                    .con th h1 {
                                        font-weight: bold;
                                        font-size: 1em;
                                        text-align: left;
                                        color: #185875;
                                    }
                                }

                                .tbl-content{
                                    height:600px;
                                    overflow-x:auto;
                                    margin-top: 0px;
                                }
                                table {
                                    width:100%;
                                    table-layout: fixed;
                                }
                                .blue { color: #185875; }
                                .yellow { color: #dabd75; }

                                .con td {
                                    font-weight: normal;
                                    font-size: 1em;
                                    -webkit-box-shadow: 0 2px 2px -2px #0E1119;
                                    -moz-box-shadow: 0 2px 2px -2px #0E1119;
                                    box-shadow: 0 2px 2px -2px #0E1119;
                                }

                                .con {
                                    text-align: left;
                                    overflow: hidden;
                                    width: 100%;
                                    margin: -15px auto;
                                    display: table;

                                }

                                .con td, .con th {
                                    padding-bottom: 2%;
                                    padding-top: 2%;
                                    padding-left:2%;
                                }

                                /* Background-color of the odd rows */
                                .con tr:nth-child(odd) {
                                    background-color: #323C50;
                                }

                                /* Background-color of the even rows */
                                .con tr:nth-child(even) {
                                    background-color: #2C3446;
                                }

                                .con th {
                                    background-color: #1F2739;
                                }

                                .con td:first-child { color: #dabd75; }

                                .con tr:hover {
                                    background-color: #464A52;
                                    -webkit-box-shadow: 0 6px 6px -6px #0E1119;
                                    -moz-box-shadow: 0 6px 6px -6px #0E1119;
                                    box-shadow: 0 6px 6px -6px #0E1119;
                                }

                                .con td:hover {
                                    background-color: #dabd75;
                                    color: #000;
                                    font-weight: bold;

                                    box-shadow: #7F7C21 -1px 1px, #7F7C21 -2px 2px, #7F7C21 -3px 3px, #7F7C21 -4px 4px, #7F7C21 -5px 5px, #7F7C21 -6px 6px;
                                    transform: translate3d(6px, -6px, 0);

                                    transition-delay: 0s;
                                    transition-duration: 0.4s;
                                    transition-property: all;
                                    transition-timing-function: line;
                                }

                                @media (max-width: 800px) {
                                    .con td:nth-child(4),
                                    .con th:nth-child(4) { display: ; }
                                }
                            </style>

                        </div>
                    </div>

                </section>
                <!-- Stop here -->
            </div>

        </div>

        <section class="section bg-white">
            <div class="container">
                <div class="nk-block">
                    <div class="row justify-content-lg-between justify-content-center align-items-center gutter-vr-30px">
                        <div class="col-lg-6 col-mb-10">
                            <div class="nk-block-img">
                                <a href="https://www.youtube.com/watch?v=Fm1pQZxbiSs" class="nk-block-video-play  video-popup btn-play btn-play-s3">
                                    <svg class="btn-play-icon-s2" viewBox="0 0 24 34"><path fill-rule="evenodd" d="M0.212,32.552 C0.427,32.851 0.769,33.010 1.117,33.010 C1.337,33.010 1.559,32.947 1.752,32.814 L23.521,17.879 C23.816,17.678 23.991,17.350 23.991,16.998 C23.991,16.646 23.816,16.319 23.521,16.115 L1.752,1.181 C1.415,0.950 0.972,0.922 0.606,1.107 C0.240,1.292 0.010,1.661 0.010,2.064 L0.010,22.480 C0.010,23.076 0.506,23.558 1.116,23.558 C1.727,23.558 2.222,23.076 2.222,22.480 L2.222,4.142 L20.963,16.998 L0.479,31.049 C-0.020,31.393 -0.140,32.066 0.212,32.552 Z"></path></svg>
                                </a>
                                <img src="../images/app-screens/sc-large-a2.png" alt="img">
                            </div>
                        </div>
                        <div class="col-lg-5 order-lg-last">
                            <div class="nk-block-text text-center text-lg-left">
                                <h2 class="title title-thin title-dark">视频说明</h2>
                            </div>
                            <p style="font-size: 1.4rem; color: #dabd75; text-align: end;">与球队 <span style="font-weight: 900; color: #000;">Globinc</span><span  style="color: #000;"></span> 不存在低迷的情况。</p>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </main>
@endsection
